...
TilesetLoader::TilesetLoader(uint32_t tileSize)
...
Texture2D TilesetLoader::get(uint32_t offsetX, uint32_t offsetY) const
{
    ...
    const auto tilesetRect = Rectangle { static_cast<float>(tilesetX), static_cast<float>(tilesetY),
                                         static_cast<float>(m_tileWidth), static_cast<float>(m_tileWidth) };
