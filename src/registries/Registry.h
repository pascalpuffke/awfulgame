...
public:
    ...
    virtual void loadAll(const std::unordered_map<std::string, T>& resources)
    {
    ...
    virtual std::optional<std::shared_ptr<T>> get(const std::string& key) const
    {
...
protected:
    std::unordered_map<std::string, std::shared_ptr<T>> m_registry;
